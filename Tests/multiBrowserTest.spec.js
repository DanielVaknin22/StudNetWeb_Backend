...
import edge from 'selenium-webdriver';
import assert from 'assert';

// Define the different browser configurations
const browsers = [
  { browserName: 'chrome', capabilities: chrome.Capabilities.chrome() },
  { browserName: 'firefox', capabilities: firefox.Capabilities.firefox() },
  { browserName: 'MicrosoftEdge', capabilities: edge.Capabilities.edge() }
];
...
async function runTest(browserConfig) {
  ...
  try {
    // Set platform and version for capabilities
    browserConfig.capabilities.set('platform', 'WINDOWS');
    browserConfig.capabilities.set('version', 'latest');

    // Initialize the WebDriver with the specified browser and capabilities
    driver = await new Builder()
      .usingServer('http://192.168.1.126:4444/wd/hub')
      .withCapabilities(browserConfig.capabilities)
      .build();
    ...
    console.log(`Test passed for ${browserConfig.browserName}`);
  } catch (e) {
    console.error(`Test failed for ${browserConfig.browserName}:`, e);
  } finally {
    if (driver) {
      await driver.quit();
    }
